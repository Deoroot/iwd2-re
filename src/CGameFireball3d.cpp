...
//
// The particle tick: advances the four quadrants along their steps, drops the
// ring temporals (m_animationID) at the live particles and retires the burst.
// Not yet recovered.
void CGameFireball3d::AIUpdate()
{
    // HACK: retire the burst on its first tick -- the explosion sound, the
    // center temporal and the registration above already happened in the
    // constructor; without this the unrecovered object would pile up in the
    // area forever. Replaces 0x711D90.
    RemoveFromArea();
}
