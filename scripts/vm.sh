...
case "$cmd" in
  ...
  smoke)
    ...
    slot=""; hold=""; hit=""; hit_min=1; expect=""; expect_min=1; ui=""; uiverdict=""
    while [ $# -gt 0 ]; do
      case "$1" in
        --hit)        hit="$2"; shift 2 ;;
    ...
    printf '@echo off\r\nREM Generated by vm.sh smoke -- do not edit.\r\ncd /d C:\\iwd2-re\r\npython scripts\\auto_start_game.py --slot %s%s > C:\\iwd2-re\\vm_s1_out.txt 2>&1\r\n' \
      "$slot" "$uiargs" > "$HERE/vm_s1_payload.cmd"
