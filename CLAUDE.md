...
- **Load/runtime crash that "closes the game itself" (no dialog) = engine top-level `__except`→`ExitProcess`; WER LocalDumps AND `frida_crash_guard` BOTH miss it (guard "DETACHED, no EXCEPTION"). Don't chase a dump.** Bisect with `Iwd2DebugLog` + self-launch `vm.sh run <slot>` and read the debug log (it writes even in the hidden session → no user needed). Last log line before death = faulting stmt; confirm vs binary. Gotcha: a crashed/stuck `iwd2-re.exe` locks `tempsave` → next launch asserts `CInfGame.cpp:787`; fix = kill strays (`(Get-Process iwd2-re).Kill()`, cross-session `taskkill` fails) + `Remove-Item tempsave -Recurse` + recreate.
