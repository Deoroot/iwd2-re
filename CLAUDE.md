...
- **Commit a clean baseline BEFORE adding `Iwd2DebugLog` instrumentation, and keep the debug as its own isolated step.** After the capture, strip it with one `git checkout`/`git revert` (or a single squashed diff) — never hand-hunt scattered debug lines later (that burns tokens locating what you added). Real recoveries go in separate commits from throwaway logging.
