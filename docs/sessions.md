...
|---|-----|------|--------|----------|
| 30 | `s30` | 2026-08-16 | no | `jumptable_audit` now DERIVES the table↔switch pairing: AMBIGUOUS-PAIRING 181 → 59, CLEAN 268 → 392, OUT-OF-RANGE 53 → 13. Five more false-positive classes (11–15), the largest being that an inlined callee's table is diffed against the callee's own recovery via `// NOTE: Uninline.`. Two real defects out of the UI triage: `CScreenWorld::CancelPopup` called `StopDeath` where the binary calls nothing (and `CancelEngine` did it for case 15, which asserts) — both missing `case 22`; and icon index 20 had no arm in `CGameDoor::SetCursor` or `CGameTrigger::SetCursor`. All four parity GREEN. |
